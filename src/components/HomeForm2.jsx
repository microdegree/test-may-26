...
function HomeSection10() {
  ...
  const numberRef2 = useRef();

  const addEmployee1 = (e) => {
    e.preventDefault();
    const Name = nameRef2.current.value;
    const Email = emailRef2.current.value;
    const Phone = numberRef2.current.value;
    base('careermentor').create(
      { Name, Email, Phone },
      function (err, record) {
        if (err) {
...
<div className="w-80 xl:w-[370px] 2xl:w-[410px] h-[510px] xl:h-[510px] 2xl:h-[560px] absolute right-4 top-3  z-10  ">
  ...
  <img className="object-fit absolute" src={rectangle40} alt="" loading="lazy" />
    ...
    <div className={`${formAlert?'block':'hidden'}  bg-opacity-90 bg-gray-900 z-30 top-0 w-full p-5 flex justify-center items-center flex-col h-full absolute space-y-4`}>
      ...
      <p className='text-sm text-white'>You have successfully submitted the form | For re-submitting <span className='text-yellow-500 cursor-pointer ' onClick={()=>setFormAlert(!formAlert)}><a href="https://www.microdegree.work/"> Click here</a></span> </p>
    </div>
    <form  action='' className=" absolute flex flex-col justify-center items-center w-full h-full p-10 px-5 space-y-3  z-20">
      ...
      <input id="exampleInputName"
            ...
            ref={nameRef} type="name" className='w-full text-sm xl:text-xl xl:p-4 2xl:text-xl text-gray-700 p-3 px-2 rounded outline-none bg-[rgba(255,255,255,0.85)]' name="" placeholder="Your name" title='Please enter your name' />
      <input type="number"
           id="exampleInputNumber"
           ref={numberRef}  className='w-full xl:text-xl text-sm xl:p-4 2xl:text-xl text-gray-700 p-2 px-3 rounded outline-none bg-[rgba(255,255,255,0.85)]' name="" pattern='[1-9]{1}[0-9]{9}' title='Please enter a 10 digit mobile no' placeholder="Mobile No" />
      <input id="exampleInputEmail1"
                    aria-describedby="emailHelp"
                    ref={emailRef} type="email" className='w-full xl:text-xl text-sm xl:p-4 2xl:text-xl text-gray-700 p-3 px-2 rounded outline-none bg-[rgba(255,255,255,0.85)]' name="" placeholder="Email" title='Please enter valid email address' />
      
      <button type="submit" onClick={addEmployee} className='bg-[#4C1BCF] mt-2 px-4 py-3 w-full  rounded text-xs  xl:p-4 xl:text-xl  text-white '>Submit Request</button>
    </form>
    </div>
  </div>
  </div>
...
<div>
  ...
  <form  action='' className=" px-5 flex flex-col justify-center items-center w-full space-y-4 ">

<input id="exampleInputName"
            aria-describedby="nameHelp"
            ref={nameRef2} type="name" className='w-full text-base p-4  text-gray-700 rounded outline-none bg-[rgba(255,255,255,0.85)]' title='Please enter your name' name="name" placeholder="Your name" />

<input type="number"
           id="exampleInputNumber"
           ref={numberRef2}  className='w-full text-base p-4  text-gray-700 rounded outline-none bg-[rgba(255,255,255,0.85)]' title='Please enter a 10 digit mobile no' name="contact" pattern='[1-9]{1}[0-9]{9}'   placeholder="Mobile No" />

<input id="exampleInputEmail1"
                    aria-describedby="emailHelp"
                    ref={emailRef2} type="email" className='w-full text-base p-4  text-gray-700 rounded outline-none bg-[rgba(255,255,255,0.85)]' title='Please enter a valid email address' name="email"  placeholder="Email" />

<button type="submit" onClick={addEmployee1} className='bg-gradient-to-br from-orange-500 to-yellow-600 mt-2 px-4 py-4 w-full  rounded text-lg  text-white '>Submit Request</button>
